...
// MOVE THE LOGIC TO THE COMPONENT 
	global $arrFilter;
	// set filter if its not (cut from this file)
	...
	global $REL_BLOCK_CODE, $REL_BLOCK_PROP;
	...
	$arSelect = array('ID', 'IBLOCK_NAME', 'NAME','CODE');
	$i = 0;
	$rsName = CIBlockElement::GetList("",$arrFilterId, false, Array(), $arSelect);
	while($ob = $rsName->GetNext()) {
		if ($partnerID == 0)
			$arrFilter['PROPERTY'][$REL_BLOCK_CODE][] = $ob['ID'];
			// $arrFilter['PROPERTY_PARTNER'][] =$ob['ID'];
	...
	$arParams["PARTNERS"] = $PARTNERS;
	$arParams["PARTNER_ID"] = $partnerID;
